...
import { Menu, X, ShoppingBag, Package, User, Search } from "lucide-react";
...
const Navbar = () => {
  ...
  // Home page gets transparent-over-hero treatment; everywhere else is solid
  const isHome = pathname === "/";
  const scrolled = useScrolled(80);
  const isTransparent = isHome && !scrolled && !mobileOpen;

  // Lock body scroll when drawer open + focus close button + ESC to close
  useEffect(() => {
  ...
  return (
    <>
        ...
        >
          ...
          <div className="flex items-center gap-1 md:gap-8 flex-1 md:flex-initial">
            <button
              ...
              className={`md:hidden inline-flex items-center justify-center w-10 h-10 -ml-2 rounded-full transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-current
      ...
      >
        ...
        {/* Panel */}
        <div
          className={`absolute inset-y-0 left-0 w-[85%] max-w-sm bg-white shadow-2xl flex flex-col transition-transform duration-500 ease-out ${
            ...
            <div className="mt-6 pt-6 border-t border-neutral-200 px-2 space-y-3">
              {!user && (
                <button
                  ...
                  className="w-full inline-flex items-center justify-center gap-2 py-3 rounded-full bg-neutral-950 text-white text-sm font-medium hover:bg-neutral-800 transition-colors"
                >
              ...
              <Link
                href="/cart"
                className="w-full inline-flex items-center justify-center gap-2 py-3 rounded-full ring-1 ring-neutral-300 text-neutral-900 text-sm font-medium hover:bg-neutral-50 transition-colors"
              >
              ...
              {isSeller && (
                <button
                  ...
                  className="w-full py-3 rounded-full ring-1 ring-amber-500 bg-amber-50 text-neutral-900 text-sm font-medium hover:bg-amber-100 transition-colors"
                >
                  Seller Dashboard
                </button>
              )}
